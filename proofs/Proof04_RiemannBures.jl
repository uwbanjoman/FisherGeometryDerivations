...
function theorem_C(; n=6, verbose=true)
    ...
    function sectional_K(a, b)
        # K(Tₐ,Tᵦ) via 4th order Bures distance
        ε4 = 5e-3
        d_ab = bures_g(ρ_star + ε4*T[a] + ε4*T[b], T[a], T[b]) +
               bures_g(ρ_star - ε4*T[a] - ε4*T[b], T[a], T[b]) -
               bures_g(ρ_star + ε4*T[a] - ε4*T[b], T[a], T[b]) -
               bures_g(ρ_star - ε4*T[a] + ε4*T[b], T[a], T[b])
        # This approximates -R_{abab}/(3ε⁴) up to corrections
        # Use direct quadratic Riemann instead:
        R_abab_quad = (n/4)^2 * sum(
            d_sym(T[b],T[a],T[f])*d_sym(T[a],T[f],T[a]) -
            d_sym(T[a],T[a],T[f])*d_sym(T[b],T[f],T[a])
            for f in 1:N)
        K = R_abab_quad / (g_val^2)
        return K
    end
    ...
    ok_C = std(K_vals) / abs(mean(K_vals)) < 0.5  # rough consistency check
